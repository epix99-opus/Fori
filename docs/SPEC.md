...
- **Agent 角色不替代**：Claude Code 限额耗尽时，不等 Codex 替代做设计/评审任务；Codex 限额耗尽时，不等 Claude Code 替代做编码任务。各自等待恢复后继续
- 限额耗尽 → 任务挂起（manifest.json status=scheduled）→ cron 在重置时间后自动续跑
